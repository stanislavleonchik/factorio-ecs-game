...
namespace Client
{
    sealed class RotationLikeParentSystem : IEcsRunSystem
    {
        ...
        public void Run(IEcsSystems systems)
        {
            foreach (var entity in filter.Value)
            {
                ...
                var parentRotation = transformPool.Value.Get(parent.value.index).value.rotation;

                transform.value.rotation = parentRotation;
            }
        }
    }
}
